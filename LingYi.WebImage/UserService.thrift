...
service UserInfoService {
	...
	GetDeviceTokenRet GetDeviceToken(1:string userBaseId,2:DeviceTypeEnum deviceType)		#根据用户编码设备类型获取Token
	...
	GetReferrerUsersRet GetReferrerUsers(1:string beginDate,2:string endDate)			#搜索关联（有推荐人）用户信息
	#GetReferrerByUserIdRet GetReferrerByUserId(1:string userBaseId)					#获取当前登录人的推荐信息
	...
	#PutUserAuthedRet PutUserAuthed(1:string userBaseId)									#通过身份证和真实姓名授权认证
	#GetUserInfosAuthRet GetUserInfosAuth(1:UserInfosAuthParam param)					#获取认证用户列表（各种认证类型的用户）
	#GetUserInfoAuthRet GetUserInfoAuthById(1:string userBaseId)							#获取认证用户详情（各种认证类型的用户）
	#GetUserAuthPassRet GetUserAuthPass(1:string userBaseId,2:UserAuthItemsEnum userAuthItemsEnum)								#认证审核通过接口
	#GetUserAuthRejectRet GetUserAuthReject(1:string userBaseId,2:UserAuthItemsEnum userAuthItemsEnum,3:string reasonStr)		#认证审核驳回接口
	...
	#GetMyFriendIsAuthRet GetMyFriendIsAuth(1:string userBaseId)							#当前用户是否认证
	#GetMyFriendIsAuthRet GetMyFriendIsAuth2(1:string userBaseId,2:string friendUserId)	#当前用户是否认证
	
	GetIsAuthed5ForEditRet GetIsAuthed5ForEdit(1:string userBaseId)						#证件信息外界是否可以反写
	Certificate5EditRet Certificate5Edit(1:string userBaseId,Certificate5Param param)	#认证模块5项数据反写接口
...
#=================认证模块5项数据反写接口=================
struct Certificate5Param{			#身份认证五项信息反写参数
	1: string trueName,				#真实姓名
	2: string identityCard,			#证件号码
	3: string imageFrontCard,		#证件正面照
	4: string imageBackCard,		#证件背面照
	5: string imageUserCard,		#证件手持照
}
...
enum Certificate5EditRet{
	...
	InvalidIdentityCard,		#无效的证件号码
	InvalidImageFrontCard,		#无效的证件正面照
	InvalidImageBackCard,		#无效的证件背面照
	InvalidImageUserCard,		#无效的证件手持照
	NoEdit,						#禁止反写
}
#=================认证模块5项数据反写接口=================
...
#=================证件信息外界是否可以反写=================
struct GetIsAuthed5ForEditRet{				#证件信息是否允许反写
	1:GetIsAuthed5ForEditStatus status,		#状态
	2:bool isCanEdit,						#true可以反写;false不允许反写
}
...
#	2:UserInfoStat userInfoStat,		#统计用到的用户信息
#	3:bool isAuth,
#}
...
#	Success,								#成功
#	InvalidUserId,							#无效的用户编码
#	InvalidUserAuthItemsEnum				#无效的认证类型项
#}
#enum GetUserAuthRejectRet{
#	Success,								#成功
#	InvalidUserId,							#无效的用户编码
#	InvalidUserAuthItemsEnum,				#无效的认证类型项 
#	InvalidReasonStr						#无效驳回原因
#}
#=================认证审核通过/驳回接口=================
...
#	1:GetUserInfoAuthStatus status,				#状态
#	2:UserInfoAuth userAuthRet,					#认证用户详情
#}
#
...
#	1:GetUserInfosAuthStatus status,			#状态
#	2:list<UserInfoAuth> usersAuthRet,			#认证用户列表
#	3:i32 totalCount,							#总计条数
#}
#enum GetUserInfosAuthStatus{
#	Success,							#成功
#	InvalidPageIndex,					#无效的PageIndex
#	InvalidPageSize,					#无效的PageSize
#	InvalidAuthedType,					#无效的认证等级
#	NoExist,							#不存在
#}
#struct UserInfoAuth{
#	1:string userBaseId,				#用户编码
#	2:string trueName,					#真实姓名
#	3:bool isTrueNameAuth,				#真实姓名是否已认证
#	4:string phone,						#电话
#	5:bool isPhoneAuth,					#电话是否已认证
#	6:string identityCard,				#身份证号
#	7:bool IsIdentityCardAuth,			#身份证是否已认证
#	8:string email,						#电邮
#	9:bool isEmailAuth,					#电邮是否已认证
#	10:string emergencyCall,			#紧急电话
#	11:bool isEmergencyCallAuth,		#紧急电话是否已认证
#	12:string address,					#联络地址
#	13:bool isAddressAuth,				#联络地址是否已认证
#	14:string imageFrontCard,			#身份证正面
#	15:bool isImageFrontCardAuth,		#身份证正面是否已认证
#	16:string imageBackCard,			#身份证背面
#	17:bool isImageBackCardAuth,		#身份证背面是否已认证
#	18:string imageUserCard,			#手持身份证
#	19:bool isImageUserCardAuth,		#手持身份证是否已认证
#	20:string loginAccount,				#账号
#	21:AgentType agentType,				#代理商类别
#	22:string marketOne,				#推荐人
#	23:string addressReject,			#地址拒绝原因
#	24:string emergencyCallReject,		#紧急电话拒绝原因
#	25:string imageBackCardReject,		#身份证背面拒绝原因
#	26:string imageFrontCardReject,		#身份证正面拒绝原因
#	27:string imageUserCardReject,		#手持身份证拒绝原因
#	28:string nickName,					#昵称
#	29:string createDateTime,			#创建时间
#	30:bool isCancel,					#账号是否注销
#	31:string authedPercent,			#认证进度
#	32:string auth3ConditionDateTime,	#
#	33:i32 memberLevel,					#会员等级
#}
#=================获取认证用户列表（各种认证类型的用户）=================
...
struct GetReferrerUsersRet{
	1: GetReferrerUsersStatus status,		#状态
	2: list<UserBase> userBases				#用户信息
}

struct UserBase{
	1: string id,							#用户编码
	2: string loginAccount,					#登录账号
	3: string loginPwd,						#登录密码
	4: string registerDateTime,				#注册时间
	5: string referrer,						#推荐人
	6: string deviceTokenCode,				#设备Token
	7: string registerDtDeviceUser,			#设备用户首次注册时间
}
enum GetReferrerUsersStatus{
	Success,								#成功
	InvalidBeginDate,						#无效的开始时间
	InvalidEndDate,							#无效的结束时间
}
