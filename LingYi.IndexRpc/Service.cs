...
namespace LingYi.IndexRpc
{
    public class Service : IService
    {
        private readonly IRpcServer _rpcServer;
        private static readonly ILog Log = CicadaFacade.CreateLog<Service>();
        private readonly string Info = "LingYi.IndexRpc Server {0} Status :{1}";
        ...
        public void Start()
        {
            try
            {
                _rpcServer.Run<IndexService.Iface>();
                Log.Info(Info, "Run", "Successful");
            }
            catch (Exception ex)
            {
                Log.Info(Info, "Start Error", ex.Message);
            }
        }
